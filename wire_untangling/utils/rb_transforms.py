# Copied from residual-offpolicy-rl (Amazon, CC-BY-NC-4.0)
# Source: resfit/rl_finetuning/utils/rb_transforms.py
#
# A MultiStep transformation for ReplayBuffers that computes n-step returns.
...
class MultiStepTransform(Transform):
    ...
    Keyword Args:
        reward_keys (list of NestedKey, optional): Defaults to ``["reward"]``.
        done_key (NestedKey, optional): Defaults to ``"done"``.
        done_keys (list of NestedKey, optional): Defaults to ``["done", "truncated", "terminated"]``.
        mask_key (NestedKey, optional): Defaults to ``"mask"``.
    """
